...
.no_dec:
    ...
    ; Poll panels periodically (only if UHMA running)
    cmp dword [rel mcp_running], 0
    je .skip_poll
    ...
    ; Preserve query_ring when a node is expanded, but keep DEBUG live
    cmp dword [rel focus_node], -1
    jne .poll_debug_only
    call poll_channels
    jmp .skip_poll
.poll_debug_only:
    call poll_debug_only
.skip_poll:

    ; Read any available data from UHMA output streams
    call read_streams

    ; Smooth auto-scroll toward write position (prevents unreadable fast scroll)
...
;; ============================================================
;; poll_debug_only — Refresh DEBUG panel without touching QUERY
;; Sends "receipts 5" to DEBUG (channel 2)
;; ============================================================
poll_debug_only:
    push rbx
    sub rsp, 8
