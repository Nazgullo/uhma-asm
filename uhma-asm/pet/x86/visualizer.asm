; visualizer.asm — UHMA Pet Command & Control Center (gui base + creature layer)
; Based on gui/visualizer.asm with creature node grafted into carousel
;
...
; @entry poll_channels() -> sends status/receipts + observe + presence via gateway subnets
...
; @calls creature.asm:creature_init, creature.asm:creature_update, creature.asm:creature_sync_uhma
; @calls render.asm:render_creature_panel
; @calledby viz_main.asm:main
;
; FEATURES:
;   - All gui/visualizer.asm features (DREAM, FEED, side panels, auto-polling)
;   - NODE_CREATURE (gold, 82°) on carousel ring with mood text
;   - Creature panel expands to 400x300 centered render_creature_panel
;   - Periodic observe cycle (every 5th poll ~15s) refreshes UHMA presence
;   - Every poll: presence sync → creature_sync_uhma updates creature state
;
; GOTCHAS:
...
section .data
    ...
    feed_start_cmd: db "./tools/feeder --cycles 1 --pause 5 &", 0
    ...
    db 82       ; NODE_CREATURE - upper-left (between BRAIN and REGIONS)
    ...
    SCROLL_STEP     equ 256               ; max bytes to advance view per frame
...
; Creature
extern creature_init
extern creature_update
extern creature_sync_uhma
extern creature_state
extern render_creature_panel
extern mood_strs
extern mood_lens
...
.skip_poll:
    ...
    ; Smooth auto-scroll toward write position
    cmp dword [rel feed_paused], 0
    jne .feed_no_sync
    mov eax, [rel feed_write]
    mov ecx, [rel feed_view]
    mov edx, eax
    sub edx, ecx
    and edx, RING_MASK
    cmp edx, SCROLL_STEP
...
.query_no_sync:
    ...
    add ecx, SCROLL_STEP
    and ecx, RING_MASK
    mov [rel debug_view], ecx
    jmp .debug_no_sync
.debug_snap:
    mov [rel debug_view], eax
.debug_no_sync:

    ; Update creature state (runs every frame even if not focused)
...
.no_menu_check:
    ...
    lea rsi, [rel feed_snap]
    call snapshot_ring
    mov dword [rel feed_snap_valid], 1
    lea rdi, [rel feed_snap]
    call copy_ring_to_clipboard
    jmp .done_click
.feed_unpause:
    ; Unpausing - snap view to latest
    mov dword [rel feed_paused], 0
    mov dword [rel feed_snap_valid], 0
    mov eax, [rel feed_write]
    mov [rel feed_view], eax
    jmp .done_click
...
.check_query_panel:
    ...
    lea rsi, [rel query_snap]
    call snapshot_ring
    mov dword [rel query_snap_valid], 1
    lea rdi, [rel query_snap]
    call copy_ring_to_clipboard
    jmp .done_click
.query_unpause:
    mov dword [rel query_paused], 0
    mov dword [rel query_snap_valid], 0
    mov eax, [rel query_write]
    mov [rel query_view], eax
    jmp .done_click
...
.check_debug_panel:
    ...
    lea rsi, [rel debug_snap]
    call snapshot_ring
    mov dword [rel debug_snap_valid], 1
    lea rdi, [rel debug_snap]
    call copy_ring_to_clipboard
    jmp .done_click
.debug_unpause:
    mov dword [rel debug_paused], 0
    mov dword [rel debug_snap_valid], 0
    mov eax, [rel debug_write]
    mov [rel debug_view], eax
    jmp .done_click
...
.send_genes:
    ...
    jmp .done_click
.send_creature:
    ; Creature node — no MCP command needed, just expand to show creature
    jmp .done_click
...
.myco_show:
    ...
    ; === CREATURE node - ring layout ===
    movzx edi, byte [rel node_angles + NODE_CREATURE]
    call calc_ring_pos
    mov r14d, eax               ; node center x
    mov r15d, edx               ; node center y
    ...
    ; Node box - warm amber for creature
    mov edi, r14d
    mov esi, r15d
    mov edx, RNODE_W
    mov ecx, RNODE_H
    ...
    mov edx, RNODE_W
    mov ecx, RNODE_H
    mov r8d, 0x00FFD700         ; gold border
    call gfx_rect

    ; Store CREATURE rect
    lea rax, [rel node_rects]
    mov [rax + NODE_CREATURE*16], r14d
    mov [rax + NODE_CREATURE*16 + 4], r15d
    mov dword [rax + NODE_CREATURE*16 + 8], RNODE_W
    mov dword [rax + NODE_CREATURE*16 + 12], RNODE_H

    ; Label centered: "CREATURE" = 8 chars
    lea edi, [r14d + 16]
    lea esi, [r15d + 18]
    lea rdx, [rel node_creature]
    mov ecx, 8
    mov r8d, [rel col_white]
    call gfx_text

    ; Show mood below label (rax not rbx — rbx holds surface_ptr)
    lea rax, [rel creature_state]
    mov eax, [rax + 84]        ; CS_MOOD
    cmp eax, 5
    jg .creature_mood_clamp
    test eax, eax
    jns .creature_mood_ok
.creature_mood_clamp:
    xor eax, eax
.creature_mood_ok:
...
.focus_creature:
    ; Render creature inside the expanded panel
    ; Panel bounds: centered in canvas, 400x300
    mov edi, CANVAS_X + CANVAS_W/2 - 200
    mov esi, CANVAS_Y + CANVAS_H/2 - 150
    mov edx, 400
    mov ecx, 300
    call render_creature_panel
    jmp .done
...
;; ============================================================
;; poll_channels — Send periodic queries + observe cycle
;; Sends "status" to QUERY, "receipts 5" to DEBUG
;; Every OBSERVE_EVERY polls: sends "observe" on FEED (updates presence)
;; Every poll: sends "presence" on QUERY → read_streams triggers creature_sync_uhma
;; ============================================================
poll_channels:
    push rbx
    ...
    ; Every OBSERVE_EVERY polls, send "observe" to refresh presence data
    inc dword [rel observe_counter]
    mov eax, [rel observe_counter]
    cmp eax, OBSERVE_EVERY
    jl .skip_observe
    mov dword [rel observe_counter], 0
    ; Send observe (fire-and-forget, output goes to feed_ring via read_streams)
    mov edi, 0                      ; FEED channel
    lea rsi, [rel cmd_observe]
    call mcp_send_async
.skip_observe:

    ; Request presence snapshot (handled in read_streams)
    mov edi, 1                      ; QUERY channel
    lea rsi, [rel cmd_presence]
    call mcp_send_async
...
.route_query:
    ; Detect presence response for creature sync
    lea rsi, [rsp]
    mov byte [rsi + r12], 0      ; null-terminate payload
    cmp byte [rsi], '-'
    jne .route_query_ring
    cmp byte [rsi + 1], '-'
    jne .route_query_ring
    cmp byte [rsi + 2], '-'
    jne .route_query_ring
...
draw_output:
    ...
    cmp dword [rel feed_paused], 0
    je .feed_live
    cmp dword [rel feed_snap_valid], 0
    je .feed_live
    lea rcx, [rel feed_snap]
.feed_live:
    ...
    cmp dword [rel query_paused], 0
    je .query_live
    cmp dword [rel query_snap_valid], 0
    je .query_live
    lea rcx, [rel query_snap]
.query_live:
    ...
    cmp dword [rel debug_paused], 0
    je .debug_live
    cmp dword [rel debug_snap_valid], 0
    je .debug_live
    lea rcx, [rel debug_snap]
.debug_live:
...
.scan_lines:
    ; Stop if we've reached the panel bottom
    mov eax, r13d
    add eax, OUT_PANEL_H - 12
    cmp r15d, eax
    jg .scan_done
    ; Copy one line from ring to stack buffer
...
.copy_line:
    ...
    cmp ecx, 42                 ; max chars per line (fit panel width)
    jge .line_done
    mov eax, ebx
    and eax, RING_MASK
...
;; ============================================================
;; snapshot_ring
;; rdi = source ring buffer, rsi = destination snapshot (8192 bytes)
;; ============================================================
snapshot_ring:
    push rbx
    mov rbx, rdi
