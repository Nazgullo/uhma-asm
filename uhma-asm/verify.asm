...
;
; @entry verify_init() -> void                      ; init verification system
; @entry verify_enable() -> void                    ; toggle on
; @entry verify_disable() -> void                   ; toggle off
; @entry verify_begin_modification(rdi=addr, esi=size) -> void ; snapshot
; @entry verify_modification(rdi=addr, esi=size) -> eax ; 1=pass, 0=fail
; @entry verify_commit() -> void                    ; discard snapshot
; @entry verify_rollback() -> void                  ; restore snapshot
; @entry verify_region_invariants(rdi=hdr) -> eax   ; 1 if invariants hold
; @entry verify_abstract(rdi=code, esi=len) -> eax  ; 1 if abstract passes
; @entry verify_geometric_gate(rdi=code, esi=len) -> eax ; 1 if geom passes
; @entry verify_unified(rdi=code, esi=len) -> eax   ; 1 if both pass
; @entry get_code_safety_score(rdi=code, esi=len) -> xmm0 ; f64 safety
; @entry verify_vsa_math() -> void                  ; startup math check
;
; @calls vsa_ops.asm:check_code_safety, factor.asm:verify_valid_call
; @calledby emit.asm, modify.asm, evolve.asm (before committing changes)
;
; GOTCHAS:
;   - Modes: 0=abstract, 1=geometric, 2=both
;   - Abstract: stack balance, callee-saved regs, valid jumps, no syscall
;   - Geometric: code→vector→dot(code, safe_template)>threshold
;   - snapshot_buf[4096] for rollback if verify fails
;
