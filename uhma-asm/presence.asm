...
;
; @entry presence_init() -> void                    ; create hardwired reflexes
; @entry trigger_presence_regions() -> void         ; evaluate all presence
; @entry presence_regions_show() -> void            ; display active regions
;
; @calls region_alloc, dream_cycle, observe_cycle
; @calledby introspect.asm:update_presence_dispatch, surface.asm:init
;
; GOTCHAS:
;   - Presence regions: PANIC_MODE, CONTEMPLATIVE, FATIGUE_RESPONSE, etc.
;   - PANIC: high entropy + low energy → fast/prune
;   - CONTEMPLATIVE: low entropy + high energy → slow down
;   - FATIGUE: high fatigue → dreams; VIGOR: high energy → exploration
;   - Thresholds: entropy(0.3-0.8), energy(20-80), arousal/fatigue(0.3-0.7)
;   - Philosophy: emotional reflexes that let system steer itself
;
