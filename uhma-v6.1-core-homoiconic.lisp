...
(defun process-chunk! (text &key verbose carry-over)
  ...
  (let ((tokens (if carry-over 
    ...
    (when (and (zerop (mod *step* 50)) (fboundp 'update-adaptive-threshold!))
      (update-adaptive-threshold!))
    ;; Self-observation: snapshot learning rate every 100 steps
    (when (and (zerop (mod *step* 100)) (> *step* *last-accuracy-snapshot*))
    ...
    ;; Program optimization (every 200 steps after warmup)
    (when (and (> *step* 500) (zerop (mod *step* 200)) (fboundp 'run-program-optimization!))
