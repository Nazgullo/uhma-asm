  ...
  (when (and (boundp '*hypotheses*) (hash-table-p *hypotheses*))
    (let ((total 0) (confirmed 0) (tested 0))
      (maphash (lambda (k h)
                 (declare (ignore k))
                 (incf total)
                 ;; Handle different hypothesis struct types
                 (cond
                   ((and (hypothesis-p h) (hypothesis-tested h))
                    (incf tested)
                    (when (hypothesis-confirmed h) (incf confirmed)))
                   ((and (self-hypothesis-p h) (> (self-hypothesis-times-tested h) 0))
                    (incf tested)
                    (when (> (self-hypothesis-confidence h) 0.7) (incf confirmed)))))
               *hypotheses*)
